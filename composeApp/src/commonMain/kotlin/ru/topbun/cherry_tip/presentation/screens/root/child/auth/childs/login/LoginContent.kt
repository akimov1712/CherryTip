...
fun LoginScreen(
    ...
    LaunchedEffect(state.loginState){
        ...
        if(loginState is LoginStore.State.LoginState.Error) errorText = loginState.errorText
    }
    ...
    Column(
        ...
        Spacer(Modifier.height(20.dp))
        ButtonLogin(
            isEnabled = isEnabledButtonLogin(state),
...
private fun isEnabledButtonLogin(state: LoginStore.State) = state.email.isNotBlank()
